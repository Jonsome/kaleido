...
export const EntryContainer = styled.div<{
  ...
  ${({ active = false, alternative = false, color: colorProp, depth, editing = false, nameValidationError, noTransition = false, rightColors, theme }) => {
    ...
    const color = colorProp || (alternative ? theme.primary : theme.templateBackgroundColor || theme.templateColor || theme.secondary)
    ...
    return css`
      ...
      font-size: 14px;
      padding: 0.4rem;
      padding-left: ${depth != null ? `calc(${depth + 1}rem - 2px)` : 'calc(1rem - 2px)'};
      padding-right: 3rem;
      color: ${theme['sideBar.foreground'] || (theme.light ? '#6c6c6c' : 'rgba(255, 255, 255, 0.5)')};
      ...
      &:hover {
        ${!(active || editing) &&
        css`
          background-color: ${theme['list.hoverBackground'] || color.clearer(0.9)()};
          color: ${theme['list.hoverForeground'] || (theme.light ? '#6c6c6c' : theme.background.lighten(5)())};
      ...
      ${active &&
      css`
        color: ${getSelectedColor(theme['list.activeSelectionForeground'])} !important;
      ...
      css`
        ...
        css`
          ...
          background-color: ${theme['list.invalidItemForeground'] || theme.redBackground.clearer(0.4)()} !important;
        `};
      `};
...
export const WorkspaceSubtitle = styled.h4`
  ${({ theme }) => css`
    ...
    margin: 0.5rem 0;
    font-weight: 400;
    color: ${theme.light ? theme.background3.darken(0.5) : theme.background3.lighten(0.5)};
    padding: 0 1rem;
  `};
`
...
export const WorkspaceInputContainer = styled.div<{ errorMessage?: string }>`
  ${({ errorMessage, theme }) => css`
    ...
    textarea {
      ...
      outline: none;
      border-radius: 2px;
      background-color: ${errorMessage ? theme.redBackground.clearer(0.5) : theme['input.background'] || 'rgba(0, 0, 0, 0.2)'};
      color: ${errorMessage ? theme.red : theme['input.foreground'] || theme.white};
